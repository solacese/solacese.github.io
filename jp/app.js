(function() {
  ...
  function populateResult() {
    ...
    var titles = {
      technology: '<em>' + iShort + '</em>のテクノロジーリーダーが取り組んでいること',
      integration: '<em>' + iShort + '</em>の統合チームがモダナイゼーションを進める方法',
      business: '<em>' + iShort + '</em>におけるリアルタイム活用のビジネスインパクト',
      architect: '<em>' + iShort + '</em>を変革するアーキテクチャパターン'
    };
    document.getElementById('resultTitle').innerHTML = titles[role] || titles.technology;

    var subtitles = {
      technology: 'あなたと同じような組織が、イベント駆動型アーキテクチャを活用してリスクを低減し、ベンダーを集約し、' + iShort + 'のデジタルトランスフォーメーションを加速する方法をご覧ください。',
      integration: iShort + 'の統合・ミドルウェアチームが、プラットフォームの集約、レガシーミドルウェアの刷新、そして迅速なデリバリーを実現する方法をご覧ください。',
      business: 'リアルタイムのイベント駆動型オペレーションが、' + iShort + 'の組織において収益を向上させ、顧客体験を改善し、競争力を変革している方法をご覧ください。',
      architect: iShort + 'の組織が Solace を活用して実践しているアーキテクチャパターン、デプロイモデル、プロトコル戦略をご覧ください。'
    };
    ...
    document.getElementById('valuesTitle').innerHTML = '<em>' + iShort + '</em>に Solace が選ばれる理由';
      ...
      sc.innerHTML = content.stories.map(function(s) {
        return '<div class="story-card">' +
          ...
          '<button class="story-expand" onclick="toggleStory(this)">' +
            '続きを読む <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>' +
          '</button>' +
          '<div class="story-detail"><div class="story-detail-inner">' + s.detail + '</div></div>' +
        '</div>';
      }).join('');
    } else if (sc) {
      sc.innerHTML = '<div class="story-card"><div class="story-headline">コンテンツを準備中です</div><div class="story-outcome">この組み合わせのストーリーを作成しています。</div></div>';
    }
      ...
      var linksHtml = content.links.map(function(l) {
        ...
        if (isGeneric) {
          return '<div class="explore-link explore-link-soon">' +
            ...
            '<span class="soon-badge">近日公開</span></div>';
        } else {
          hasLiveLinks = true;
  ...
  function renderArchitecture(industry) {
    ...
    leftEl.innerHTML = '<div class="col-label">オペレーショナルテクノロジー</div><div class="col-nodes">' +
    ...
    rightEl.innerHTML = '<div class="col-label">エンタープライズ・コンシューマー</div><div class="col-nodes">' +
  ...
  function renderLogos(industry) {
    ...
    document.getElementById('logoLabel').textContent = iShort + 'のリーダー企業に信頼されています';
  ...
  function toggleStory(btn) {
    ...
    btn.innerHTML = btn.classList.contains('open')
      ? '折りたたむ <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>'
      : '続きを読む <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>';
  }
